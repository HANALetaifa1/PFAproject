...
const refreshTokenSecret = process.env.REFRESH;

router.post('/register', uploadFile.single("avatar"),async (req, res) =>  { 
    try { 
        //ajou****
    let { email, password, firstName, lastName,   phone ,  datenais,role  } = req.body 
    const avatar =req.file.filename 
     const user = await User.findOne({ email }) 
             if (user) return res.status(404).send({ success: false, message: "L'utilisateur existe déjà" }) 
      
             const newUser = new User({ email, password, firstName, lastName,avatar ,phone,datenais,role }) 
    ...
    var mailOption ={ 
      ...
      <a href="http://${req.headers.host}/user/api/status/edit?email=${newUser.email}"> Cliquez ici </a>` 
    } 
